...
const AddQuestion = ({ isOpen, onClose, id }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      await addNewQuestionToAssessment(id, questionData);
      setIsLoading(false);
      reset();
      onClose();
    } catch (error) {
    ...
    isOpen && (
      <div className="fixed inset-0 z-50 flex justify-center items-center bg-gray-600 bg-opacity-50">
        <ListContainer className="bg-white rounded-xl p-5 w-1/2 overflow-y-auto max-h-[95vh]">
          <div onSubmit={handleSubmit} className="flex flex-col gap-3 p-2">
            ...
            <input
              ...
              onChange={(e) =>
                setQuestionData({ ...questionData, points: e.target.value })
              }
